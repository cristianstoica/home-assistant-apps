...
representative entity `sensor.wu_temp_<key>`. The transform is **pure** — no I/O,
no logging — so it is fully oracle-testable against a `/states` fixture. It
...
* The representative shape is the anchored `^sensor\\.wu_temp_([a-z0-9]+)$` — the
  capture group is the station `key`. The match anchors on the `temp` metric
  specifically because `sensor.wu_temp_<key>` is the entity the poller drives.
* `update_entity` = `sensor.wu_temp_<key>`; `expected_sensors` = the count of
  `sensor.wu_*_<key>` siblings present in `/states` (via `health.discover`) — a
  soft, logged-only first-boot snapshot, never a health gate.
* Any `sensor.wu_temp_*` whose suffix is NOT bare lowercase-alphanumeric (an
  underscore or uppercase char, e.g. `sensor.wu_temp_back_yard`) is SKIPPED into
  `skipped_entity_ids` — preserving the strict `^[a-z0-9]+$` key contract rather
  than weakening it.
* Keys are de-duplicated (a key can only match `sensor.wu_temp_<key>` once).
"""
...
# `sensor.wu_temp_` then a bare lowercase-alphanumeric suffix, anchored both ends.
_REPRESENTATIVE_RE = re.compile(r"^sensor\.wu_temp_([a-z0-9]+)$")
# The looser prefix that flags a non-conforming representative for the skip list:
# it looks like a temp representative but its suffix fails the strict key shape.
_TEMP_PREFIX = "sensor.wu_temp_"
...
class DiscoveryResult(NamedTuple):
    ...
    `skipped_entity_ids` is every `sensor.wu_temp_*` entity whose suffix failed
...
def discover_stations(states: list[EntityState]) -> DiscoveryResult:
    ...
    key (sibling count via `health.discover`), and collects any `sensor.wu_temp_*`
    with a non-conforming suffix into `skipped_entity_ids`. Returns both; logs
    nothing.
    """
    stations: list[Station] = []
    skipped: list[str] = []
    ...
    for entity in states:
        ...
        if match is None:
            if eid.startswith(_TEMP_PREFIX):
                # Looks like a temp representative but the suffix is not a bare
                # lowercase-alphanumeric key — skip (preserve the key contract).
                skipped.append(eid)
            continue
        key = match.group(1)
        if key in seen:
